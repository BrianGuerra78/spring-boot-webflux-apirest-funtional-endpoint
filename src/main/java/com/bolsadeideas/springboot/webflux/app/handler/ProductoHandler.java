...
public class ProductoHandler {
		...
		return request.multipartData().map(multipart -> multipart.toSingleValueMap().get("file")).cast(FilePart.class)
				.flatMap(file -> producto.flatMap(p -> {
					...
					return file.transferTo(new File(path + p.getFoto())).then(service.save(p));
						...
						.contentType(MediaType.APPLICATION_JSON).body(fromObject(p)));
	}
	...
	public Mono<ServerResponse> editar(ServerRequest request) {
		...
		/*
		 * Mono<Producto> producto = request.bodyToMono(Producto.class); String id =
		 * request.pathVariable("id");
		 * 
		 * Mono<Producto> productoDb = service.findById(id);
		 * 
		 * return productoDb.zipWith(producto, (db, req) -> {
		 * db.setNombre(req.getNombre()); db.setPrecio(req.getPrecio());
		 * db.setCategoria(req.getCategoria()); return db; }).flatMap(p ->
		 * ServerResponse.created(URI.create("/api/v2/productos/".concat(p.getId())))
		 * .contentType(MediaType.APPLICATION_JSON).body(service.save(p),
		 * Producto.class) .switchIfEmpty(ServerResponse.notFound().build()));
		 */
		return request.bodyToMono(Producto.class).flatMap(preducto -> service.save(preducto))
				.flatMap(prod -> ServerResponse.ok().contentType(MediaType.APPLICATION_JSON).bodyValue((prod))
						.switchIfEmpty(ServerResponse.notFound().build()));
	}
